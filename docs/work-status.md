...
- 사용자 정보(이름·학과·포인트 등)를 JS 상수에 박아두기좌측 하단 텍스트 영역 (콘텐츠 및 스타일 변경):

기존의 굵은 검은색 텍스트 "우리의 상상은 현실로"를 제거합니다.

당신이 제공한 특수 스타일 브랜드 텍스트 이미지를 더 크고 중앙에 위치하게 배치합니다.

우측 디자인 영역 (콘텐츠 통합 및 배경 제거):

기존 체크무늬 배경의 카드 프레임을 완전히 제거합니다.

그 카드 안에 있던 모든 일러스트 내용물 (세 가지 일러스트 제품, 대형 연필, 모든 소형 일러스트 아이콘, 그리고 그 안에 있던 브랜드 문구와 하단 영어 텍스트)을 깨끗한 배경에 직접 배치합니다.

모든 요소가 부드럽게 통합되도록 정렬하고, 자연스러운 조명을 추가합니다.
...
| # | 작업 | 의존 |
|---|---|---|
| 1 | **펀드 개설 화면** (`fund-create.html` + `fund-create.js`) | A의 `POST /api/funds` 가 먼저 동작해야 함 |
| 2 | **AI 옷 디자인 생성** (Bedrock / Stable Diffusion / FLUX 검토) | 외부 API 결정 후 |
| 3 | **AI 모델 피팅** (FASHN AI / Leffa / IDM-VTON) | GPU/비용 결정 후 |
...
### A-4. TypeScript 타입 + Repository (8개 엔티티)
...
- A-4-a: `Product`
- A-4-b: `Design`
- A-4-c: `Fund` (참고: snake_case ↔ camelCase 매핑 신중히)
- A-4-d: `FundParticipation`
- A-4-e: `FundLike`
- A-4-f: `Comment`
- A-4-g: `Payment`
- A-4-h: `Notification`
...
| POST | `/api/payments/confirm` | ✓ | 결제 (participationId, method) |
...
> **B의 미션**: "모든 mock 데이터를 실제 서버 API 호출로 바꾸고, 누락된 화면을 채운다."
> A의 작업에 의존하지만, B-1·B-2(공용 헬퍼)는 즉시 시작 가능.
...
### B-3. `MOCK_USER` 제거 + `/api/auth/me` 연동
...
### B-4. `MOCK_PRODUCTS` 제거 — 페이지별 API 연동
...
| `detail.js` | `MOCK_PRODUCTS.find(p => p.id === id)` | `await api.get('/funds/' + encodeURIComponent(id))` |
| `payment.js` | 동일 | 동일 |
...
**의존**: A-5의 `/api/me/addresses/*`.

---

### B-8. 결제 수단 관리 페이지 (범위 제한)
...
**의존**: A-5의 `/api/notifications/*`.

---

### B-10. 푸시 알림 (FCM) 클라이언트 통합
...
2. `frontend/push.js` (신규)
   ...
   - FCM 토큰 발급
   - `POST /api/me/push-tokens` 로 서버에 토큰 저장
3. **A에게 요청**할 백엔드 작업: `push_token` 테이블 + `POST /api/me/push-tokens` 엔드포인트

이번 분담에서는 **클라이언트 토큰 발급·저장까지만**. 서버 → FCM 전송은 별도.

---

### B-11. UX 정리 — `alert/confirm` → 토스트
...
- **A**: A-1(마이그레이션) → A-2(migrate 스크립트) → A-3(미들웨어/DI) → A-4 일부(`Product`, `Fund`, `FundParticipation`)
- **B**: B-1(api.js) → B-2(dom.js) → B-11(toast.js) — 즉시 가능
- **사장님**: 대기 (A-1~A-5의 `POST /api/funds` 가 끝나야 시작)

### 2주차

- **A**: A-4 나머지 → A-5(API 엔드포인트들) → A-6(me 확장) → A-7(결제) → A-8(검색)
- **B**: B-3(`MOCK_USER` 제거) → B-4(`MOCK_PRODUCTS` 제거 — 페이지별 순차) → B-5(댓글)
- **사장님**: 펀드 개설 화면 시작

### 3주차

- **A**: 버그 수정 + 인덱스 최적화
- **B**: B-6(마이페이지 통계) → B-7(배송지) → B-8(결제수단 표시) → B-9(알림 서버화) → B-10(FCM)
- **사장님**: AI 디자인 생성 시작
...
A-1 마이그레이션
 └→ A-2 migrate 스크립트
     └→ A-3 DI 와이어링
         └→ A-4 타입·Repository
             └→ A-5 API 엔드포인트
                 ├→ 사장님: 펀드 개설 화면 (POST /api/funds 의존)
                 ├→ B-4 MOCK_PRODUCTS 제거
                 ├→ B-5 댓글
                 ├→ B-6 마이페이지 통계
                 ├→ B-7 배송지
                 └→ B-9 알림

A-6 me 확장 → B-3 MOCK_USER 제거

B-1 api.js ← 모든 B 작업 전제
B-2 dom.js ← B-4·B-5 전제
```
...
```
이 문서의 [A-4-a Product 엔티티] 작업을 진행해줘.
...
- PR은 작업 단위(A-1, A-2, B-1...)로 분리. 한 PR에 여러 작업 묶지 말 것
- 머지 전 두 모드에서 동작 확인:
